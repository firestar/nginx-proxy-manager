// Regex matching the `proxy` log_format:
...
/**
 * Parse a single nginx `proxy` format log line.
 *
 * @param   {string} line
 * @returns {{bucket: string, status: number, bytes: number, cacheHit: boolean}|null}
 */
export const parseLogLine = (line) => {
	...
	// time_local format: 11/Jul/2026:14:30:00 +0000
	const timeParts =
	...
	// Apply the ±ZZZZ offset so buckets are UTC regardless of server timezone
